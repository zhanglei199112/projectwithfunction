...
public class PartTree implements Iterable<OrPart> {
   ...
   *
   * an upper-case letter that has a lower-case variant \p{Lu} OR any other letter NOT in the
   * BASIC_LATIN Uni-code Block \\P{InBASIC_LATIN} (like Chinese, Korean, Japanese, etc.).
   *
   * @see <a
   * href="http://www.regular-expressions.info/unicode.html">http://www.regular-expressions.info/
   * unicode.html</a>
   * 
   * @see <a
   * href="http://docs.oracle.com/javase/7/docs/api/java/util/regex/Pattern.html#ubc">Pattern</a>
   */
  ...
  /**
   * The subject, for example "findDistinctUserByNameOrderByAge" would have the predicate
   * "NameOrderByAge".
   */
  ...
  /**
   * Splits the given text at the given keywords. Expects camel-case style to only match concrete
   * keywords and not derivatives of it.
   * 
  ...
  /**
   * Represents the subject part of the query. E.g. {@code findDistinctUserByNameOrderByAge} would
   * have the subject {@code DistinctUser}.
   * 
   * @author Phil Webb
  ...
  private static class Subject {
    ...
    private static final Pattern EXISTS_BY_TEMPLATE =
        Pattern.compile("^(" + EXISTS_PATTERN + ")(\\p{Lu}.*?)??By");
    private static final Pattern DELETE_BY_TEMPLATE =
        Pattern.compile("^(" + DELETE_PATTERN + ")(\\p{Lu}.*?)??By");
    private static final String LIMITING_QUERY_PATTERN = "(First|Top)(\\d*)?";
    private static final Pattern LIMITED_QUERY_TEMPLATE = Pattern.compile("^(" + QUERY_PATTERN
    ...
    // 判断是否为查询
    private static final Pattern QUERY_BY_TEMPLATE =
        Pattern.compile("^(" + QUERY_PATTERN + ")(\\p{Lu}.*?)??By");
    ...
    private String findQueryProperty(String subject) {
      ...
      if (matches(subject, QUERY_BY_TEMPLATE)) {
        String property = Pattern.compile("^(" + QUERY_PATTERN + ")|By$", Pattern.DOTALL)
            .matcher(subject).replaceAll("");
