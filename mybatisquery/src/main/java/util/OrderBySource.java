...
import util.Sort.Direction;
import util.Sort.Order;
...
class OrderBySource {
	...
	private static final Set<String> DIRECTION_KEYWORDS = new HashSet<String>(
	...
	public OrderBySource(String clause) {
		...
		for (String part : clause.split(BLOCK_SPLIT)) {
			...
			this.orders.add(createOrder(propertyString, direction));
		}
	}
